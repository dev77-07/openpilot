...
class SpeedLimitRenderer(Widget):
  """Speed limit display widget using pfeiferj/mapd data."""
  ...
  def _update_state(self) -> None:
    ...
    # Get data from pfeiferj/mapd via mapdOut
    if sm.valid["mapdOut"]:
      mapd = sm["mapdOut"]
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    ...
    sign_y = rect.y + rect.height / 2 - sign_height / 2
    ...
    # Draw upcoming speed limit if different
  ...
  def _draw_vienna_sign(self, sign_rect: rl.Rectangle, speed_str: str, sub_text: str, speed_color: rl.Color, has_speed_limit: bool) -> None:
    """Draw EU/Vienna Convention style circular sign."""
  ...
  def _draw_upcoming_speed_limit(self, sign_rect: rl.Rectangle) -> None:
    ...
    distance_pos = rl.Vector2(ahead_rect.x + (ahead_rect.width - distance_size.x) / 2, ahead_rect.y + 110)
